...
template <class T>
std::shared_ptr<T> TreeItem<T>::getValue() const
{
	return this->figure;
}
...
template std::ostream& operator<<(std::ostream& os, const TreeItem<Rectangle>& tree);
template class TreeItem<Figure>;
template std::ostream& operator<<(std::ostream& os, const TreeItem<Figure>& tree);